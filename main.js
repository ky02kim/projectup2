...
function makeBwMonthTotals() {
    return state.bw.filtered.reduce((acc, r) => {
        const y = r.send_date?.substring(0, 4);
        const m = r.send_date?.substring(5, 7);   // "01"~"12"
        if (!y || !m) return acc;                 // 날짜가 없으면 무시

        const key = `${y}-${m}`;
        const add = Number(r.jong_count || 0) + Number(r.ye_count || 0);
        acc[key] = (acc[key] || 0) + add;          // 같은 월이면 누적
        return acc;
    }, {});   // → { "2016-01": 100, "2016-02": 87, … }
}

function renderVisCombinedChart() {
    /* ---------- 기존 코드 (그대로 유지) ---------- */
    const ymBw  = state.bw.filtered
        .map(r => {
            const y = r.send_date?.substring(0,4),
                  m = r.send_date?.substring(5,7);
            return y && m ? `${y}-${m}` : null;
        })
        .filter(Boolean);
    ...
    /* ---------- 새로 만든 월‑별 합계 객체 ---------- */
    const bwMonthTotals = makeBwMonthTotals();   // { "2016-01":100, … }
    ...
    /* ---------- 전체 합계도 화면에 표시 (선택) ---------- */
    const totalBw = Object.values(bwMonthTotals).reduce((s, v) => s + v, 0);
    // 예시 :  에 전체 합계 삽입
    setEl('up_bw_total', totalBw);
}
