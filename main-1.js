...
function initBwYearFilter() {
    document.getElementById('upBwYearFilter').addEventListener('click', e=>{
        ...
        renderBwUploadSection(btn.dataset.year==='all'?'all':btn.dataset.year);
    });
}
...
function renderBwReportTable(yearFilter='all') {
    ...
    tbody.innerHTML = rows.map(r=>{
        ...
        return `<tr>
            ...
            <td>${badge}</td>
            <td>${period}</td>
            <td style="font-family:var(--mono);text-align:center">${r.jongTarget}/${r.jongComplete}</td>
            <td style="font-family:var(--mono);text-align:center">${r.yeTarget}/${r.yeComplete}</td>
            <td style="font-family:var(--mono);text-align:center;color:var(--teal);font-weight:700">${r.total}</td>
        </tr>`;
    }).join('');
}

// 사후관리 상세: 원자료(사업장 단위) 목록
function renderBwDetailTable(yearFilter='all') {
    ...
    setEl('bwDetailRowCount', data.length+'건');
    const tbody=document.getElementById('bwDetailTableBody');
    if(!data.length){ tbody.innerHTML=`<tr><td colspan="9"><div class="empty-state sm"><i class="fas fa-file-medical"></i><p>데이터가 없습니다</p></div></td></tr>`; return; }
