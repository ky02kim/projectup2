...
const state = {
    ...
    hgc:  { raw:[], filtered:[], yearFilter:'all', detailPage:1, detailSearch:'' },
    etc:  { biman:[], swi:[], ombu:[] },
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    loadHgcData();         // window.HGC_DATA 로드 (정기 사후관리 발송)
    loadEtcData();
});
...
function updateVisYearFilterButtons() {
    const years = new Set([
        ...
        ...state.hgc.raw.map(r=>parseInt(r.periodStart?.substring(0,4))||0)
    ].filter(y=>y>2000));
...
function applyVisFilters() {
    ...
    state.hgc.filtered = state.hgc.raw.filter(r=>{
        if (year!=='all' && r.periodStart?.substring(0,4)!==String(year)) return false;
        return true;
    });
    renderVisAll();
}
...
// A. 정기 발송 (hgc.js 데이터 기반)
function renderVisBwKPI() {
    const curY = new Date().getFullYear();
    const yearData = state.hgc.raw.filter(r=>r.periodStart?.startsWith(String(curY)));
    const completeYear = yearData.filter(r=>r.status==='complete');
    const periods = new Set(completeYear.map(r=>`${r.periodStart}_${r.periodEnd}`));
    setEl('v_bw_total',   periods.size);
    setEl('v_bw_sogyeon', completeYear.length);
    setEl('v_bw_noesim',  completeYear.filter(r=>Array.isArray(r.items)&&(r.items[0]||r.items[1])).length);
    const latest = state.hgc.raw.filter(r=>r.status==='complete').map(r=>r.fileDate).filter(Boolean).sort().pop();
    setEl('v_bw_month', latest || '-');
}
function renderVisBwCharts() {
    const data = state.hgc.filtered.filter(r=>r.status==='complete');
    const months=Array.from({length:12},(_,i)=>`${i+1}월`);
    const ye = Array(12).fill(0);
    const jong = Array(12).fill(0);
    data.forEach(r=>{
        const m=parseInt(r.periodStart?.substring(5,7)||'')-1;
        if(isNaN(m)||m<0||m>11) return;
        const jy=parseJY(r.jongYe);
        jong[m]+=jy.jong; ye[m]+=jy.ye;
    });
...
// D. 종합 비교
function makeBwMonthTotals() {
    return state.hgc.filtered.filter(r=>r.status==='complete').reduce((acc, r) => {
        const key = r.periodStart?.substring(0, 7);   // "YYYY-MM"
        if (!key) return acc;
        acc[key] = (acc[key] || 0) + 1;
        return acc;
    }, {});   // → { "2026-01": 12, "2026-02": 9, … }
}

function renderVisCombinedChart() {
    const ymBw = state.hgc.filtered
        .filter(r=>r.status==='complete')
        .map(r=>r.periodStart?.substring(0,7))
        .filter(Boolean);
    ...
    /* ---------- 월‑별 합계 객체 ---------- */
    const bwMonthTotals = makeBwMonthTotals();   // { "2026-01":12, … }

    /* ---------- 정기 발송 데이터(bwD)를 **합계**로 교체 ---------- */
    const bwD = ymSet.map(ym => bwMonthTotals[ym] ?? 0);   // 존재하지 않으면 0

    /* ---------- 나머지는 기존대로 ---------- */
    const knxD = ymSet.map(ym => {
        const [y, m] = ym.split('-').map(Number);
        return state.knx.filtered.filter(r => r.y === y && r.m === m).length;
    });
    const reqD = ymSet.map(ym => {
        const [y, m] = ym.split('-').map(Number);
        return state.req.filtered.filter(r => r.y === y && r.m === m).length;
    });
    ...
    CH.v_combined = safeChart('v_combinedChart', {
        ...
        data: {
            labels: ymSet,
            datasets: [
                { label: '정기 발송', data: bwD,
                  borderColor: C.green,
                  backgroundColor: 'rgba(22,163,74,.10)', fill: true,
                  tension: .4, pointRadius: 4, pointBackgroundColor: C.green },

                { label: 'Knox 발송', data: knxD,
                  borderColor: C.blue,
                  backgroundColor: 'rgba(30,115,230,.10)', fill: true,
                  tension: .4, pointRadius: 4, pointBackgroundColor: C.blue },

                { label: '자료요청', data: reqD,
                  borderColor: C.orange,
                  backgroundColor: 'rgba(234,88,12,.10)', fill: true,
                  tension: .4, pointRadius: 4, pointBackgroundColor: C.orange },
            ]
        },
        options: lineOpts()
    });
...
function updateDataStatus() {
    ...
    if (knxOk&&reqOk) {
        ...
        text.innerHTML=`<span class="status-dot loaded"></span> Knox ${state.knx.raw.length}건 · 요청서 ${state.req.raw.length}건`;
    } else if (knxOk||reqOk) {
...
function loadExternalData() {
    ...
    console.log(`[DATA] Knox ${state.knx.raw.length}건, REQ ${state.req.raw.length}건 로드`);
...
/* =====================================================
   데이터 입력 탭 — 정기 발송 (hgc.js 자동 집계)
   ===================================================== */
function initBwYearFilter() {
    document.getElementById('upBwYearFilter').addEventListener('click', e=>{
        const btn=e.target.closest('.filter-btn'); if(!btn) return;
        toggleActive(document.getElementById('upBwYearFilter'), btn);
        state.hgc.yearFilter = btn.dataset.year==='all'?'all':btn.dataset.year;
        state.hgc.detailPage = 1;
        renderBwUploadSection(state.hgc.yearFilter);
    });
    document.getElementById('bwDetailSearch').addEventListener('input', e=>{
        state.hgc.detailSearch = e.target.value.toLowerCase();
        state.hgc.detailPage = 1;
        renderBwDetailTable(state.hgc.yearFilter);
    });
}
...
function loadHgcData() {
    state.hgc.raw = (window.HGC_DATA || []).filter(r => r && r.periodStart && r.periodEnd);

    const years = [...new Set(state.hgc.raw.map(r=>r.periodStart.substring(0,4)))].sort();
    const yf = document.getElementById('upBwYearFilter');
    yf.innerHTML = '<button class="filter-btn active" data-year="all">전체</button>';
    ...
    state.hgc.yearFilter = 'all';
    state.hgc.detailPage = 1;
    state.hgc.detailSearch = '';
    renderBwUploadSection('all');
    updateVisYearFilterButtons();
    applyVisFilters();
}

// periodStart/periodEnd 기준으로 그룹핑
function groupHgcByPeriod(data) {
    const map = {};
    data.forEach(r=>{
        const key = `${r.periodStart}_${r.periodEnd}`;
        if(!map[key]) map[key] = { periodStart:r.periodStart, periodEnd:r.periodEnd, records:[] };
        map[key].records.push(r);
    });
    return Object.values(map).sort((a,b)=>b.periodStart.localeCompare(a.periodStart));
}

// 레코드 묶음 → 종건/예건 target·complete 집계
function summarizeHgcRecords(records) {
    const jongTarget   = records.filter(r=>parseJY(r.jongYe).jong && r.status==='target').length;
    const jongComplete = records.filter(r=>parseJY(r.jongYe).jong && r.status==='complete').length;
    const yeTarget      = records.filter(r=>parseJY(r.jongYe).ye && r.status==='target').length;
    const yeComplete    = records.filter(r=>parseJY(r.jongYe).ye && r.status==='complete').length;
    const total = records.length;
    const completeDates = records.filter(r=>r.status==='complete').map(r=>r.fileDate).filter(Boolean).sort();
    const anyDates = records.map(r=>r.fileDate).filter(Boolean).sort();
    const sendDate = completeDates.pop() || anyDates.pop() || '-';
    return { jongTarget, jongComplete, yeTarget, yeComplete, total, sendDate };
}

function renderBwUploadSection(yearFilter='all') {
    renderBwReportTable(yearFilter);
    renderBwDetailTable(yearFilter);
}

// 발송 보고 목록: 대상기간별로 그룹핑 → 사후관리 소견서(전체) / 뇌심·직무(items[0|1]) 행 생성
function renderBwReportTable(yearFilter='all') {
    const data = state.hgc.raw.filter(r=>yearFilter==='all'||r.periodStart?.startsWith(String(yearFilter)));
    const groups = groupHgcByPeriod(data);
    const rows = [];
    groups.forEach(g=>{
        rows.push({ type:'사후관리 소견서', periodStart:g.periodStart, periodEnd:g.periodEnd, ...summarizeHgcRecords(g.records) });
        const noesimRecords = g.records.filter(r=>Array.isArray(r.items)&&(r.items[0]||r.items[1]));
        if(noesimRecords.length) {
            rows.push({ type:'뇌심·직무', periodStart:g.periodStart, periodEnd:g.periodEnd, ...summarizeHgcRecords(noesimRecords) });
        }
    });
    setEl('bwRowCount', rows.length+'건');
    const tbody=document.getElementById('biweeklyTableBody');
    if(!rows.length){ tbody.innerHTML=`<tr><td colspan="6"><div class="empty-state sm"><i class="fas fa-file-medical"></i><p>데이터가 없습니다</p></div></td></tr>`; return; }
    tbody.innerHTML = rows.map(r=>{
        const badge = r.type==='사후관리 소견서' ? '<span class="badge b-sogyeon">사후관리 소견서</span>' : '<span class="badge b-noesim">뇌심·직무</span>';
        const period = `${(r.periodStart||'').substring(5)} ~ ${(r.periodEnd||'').substring(5)}`;
        return `<tr>
            <td style="font-family:var(--mono);color:var(--blue)">${r.sendDate}</td>
            <td>${badge}</td>
            <td>${period}</td>
            <td style="font-family:var(--mono);text-align:center">${r.jongComplete}/${r.jongTarget}</td>
            <td style="font-family:var(--mono);text-align:center">${r.yeComplete}/${r.yeTarget}</td>
            <td style="font-family:var(--mono);text-align:center;color:var(--teal);font-weight:700">${r.total}</td>
        </tr>`;
    }).join('');
}

// 사후관리 상세: 원자료(사업장 단위) 목록 (검색 + 페이지네이션)
function renderBwDetailTable(yearFilter='all') {
    let data = state.hgc.raw.filter(r=>yearFilter==='all'||r.periodStart?.startsWith(String(yearFilter)));
    if(state.hgc.detailSearch){
        const q=state.hgc.detailSearch;
        data = data.filter(r=>
            (r.fileDate||'').includes(q) ||
            (r.workplace||'').toLowerCase().includes(q) ||
            (r.jongYe||'').toLowerCase().includes(q) ||
            (r.status||'').toLowerCase().includes(q)
        );
    }
    data = [...data].sort((a,b)=>(b.fileDate||'').localeCompare(a.fileDate||''));
    setEl('bwDetailRowCount', data.length.toLocaleString()+'건');
    const total = Math.ceil(data.length/PAGE)||1;
    if(state.hgc.detailPage>total) state.hgc.detailPage=total;
    const page = data.slice((state.hgc.detailPage-1)*PAGE, state.hgc.detailPage*PAGE);
    const tbody=document.getElementById('bwDetailTableBody');
    if(!page.length){ tbody.innerHTML=`<tr><td colspan="9"><div class="empty-state sm"><i class="fas fa-file-medical"></i><p>데이터가 없습니다</p></div></td></tr>`; renderPagination('bwDetailPagination', total, state.hgc.detailPage, p=>{ state.hgc.detailPage=p; renderBwDetailTable(yearFilter); }); return; }
    tbody.innerHTML = page.map(r=>{
        const period = `${(r.periodStart||'').substring(5)} ~ ${(r.periodEnd||'').substring(5)}`;
        const statusBadge = r.status==='complete' ? '<span class="badge b-sogyeon">완료</span>' : '<span class="badge b-noesim">대상</span>';
        const items = Array.isArray(r.items)?r.items:[];
        return `<tr>
            <td style="font-family:var(--mono);color:var(--blue)">${r.fileDate||'-'}</td>
            <td>${period}</td>
            <td style="max-width:140px;overflow:hidden;text-overflow:ellipsis" title="${r.workplace||''}">${r.workplace||'-'}</td>
            <td>${r.jongYe?jongYeBadge(r.jongYe):'<span class="badge b-etc">-</span>'}</td>
            <td>${statusBadge}</td>
            <td style="text-align:center">${ck(items[0])}</td>
            <td style="text-align:center">${ck(items[1])}</td>
            <td style="text-align:center">${ck(items[2])}</td>
            <td style="text-align:center">${ck(items[3])}</td>
        </tr>`;
    }).join('');
    renderPagination('bwDetailPagination', total, state.hgc.detailPage, p=>{ state.hgc.detailPage=p; renderBwDetailTable(yearFilter); });
}
